...
export const get: RequestHandler = async () => {
    ...
    await (await mongoClient).db()?.collection('locations').insertMany(locations.map(l => ({name: l.name, coordinates: l.coordinates})));

    let savedLocations = await (await mongoClient).db()?.collection('locations').find({}).toArray()
    savedLocations = savedLocations.map(l => ({...l, id: locations.find(ll => ll.name === l.name)?.id}))
    ...
    await Promise.all(
        sports.map(async sport => (await (await mongoClient).db()?.collection('sports').updateOne({_id: sport._id}, {$set: sport})))
    );
