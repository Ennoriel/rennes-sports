...
export function filterSports(filter: Filter, sports: Array<Sport>): Array<Sport> {
	return sports.filter(sport => !filter.sport || sport.sport === filter.sport)
		...
		.filter(sport => !filter.locationId || sport.slots.some(slot => slot.locationId === filter.locationId))
...
export function getMarkers(sports: Array<Sport>, locations: Array<Location>): any {
	return locations
		...
		.filter(location => sports.some(sport => sport.slots.some(slot => slot.locationId === location.id)))
		.map(location => ({
			location,
